...
export async function init(dir: string) {
  ...
  // Guard: don't overwrite existing scaffold.yml
  const ymlPath = join(targetDir, "scaffold.yml");
  if (existsSync(ymlPath)) {
    console.warn("scaffold.yml already exists — skipping init (delete it to re-initialize)");
    return;
  }

  // Write scaffold.yml
  await Bun.write(ymlPath, STARTER_YAML);
  console.log("  Created scaffold.yml");
  ...
  console.log("  1. Edit scaffold.yml to define your data entities");
